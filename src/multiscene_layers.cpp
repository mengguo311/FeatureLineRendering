// Read-only per-pixel contribution layers from pinned native rasterizer state.
#include <algorithm>
#include <cmath>
#include <cstdint>

extern "C" void multiscene_layers(int h,int w,const float* xy,const float* conic,
    const float* depth,const uint32_t* ids,const uint32_t* ranges,
    int64_t* counts,const int64_t* offsets,float* zs,float* weights,float* transmittance) {
    #pragma omp parallel for schedule(static)
    for(int p=0;p<h*w;++p) {
        int x=p%w,y=p/w,tile=(y/16)*((w+15)/16)+x/16;
        float T=1;int64_t n=0;
        for(uint32_t j=ranges[2*tile];j<ranges[2*tile+1];++j) {
            auto i=ids[j];float dx=xy[2*i]-x,dy=xy[2*i+1]-y;
            const float* co=conic+4*i;
            float power=-.5f*(co[0]*dx*dx+co[2]*dy*dy)-co[1]*dx*dy;
            if(power>0)continue;
            float a=std::min(.99f,co[3]*std::exp(power));
            if(a<1.f/255.f)continue;
            float next=T*(1-a);
            if(next<.0001f)break;
            if(zs){auto k=offsets[p]+n;zs[k]=depth[i];weights[k]=a*T;transmittance[k]=next;}
            ++n;T=next;
        }
        counts[p]=n;
    }
}

extern "C" void multiscene_layer_query(int n,int pixels,const int64_t* offsets,
    const float* zs,const float* ts,const int64_t* pixel,const double* depth,
    double delta,double* front,uint8_t* support) {
    #pragma omp parallel for schedule(static)
    for(int i=0;i<n;++i) {
        support[i]=0;front[i]=1;
        if(pixel[i]<0||pixel[i]>=pixels||!std::isfinite(depth[i]))continue;
        auto begin=offsets[pixel[i]],end=offsets[pixel[i]+1];
        if(begin==end)continue;
        const float* f=std::lower_bound(zs+begin,zs+end,depth[i]-2*delta);
        if(f>zs+begin)front[i]=ts[f-zs-1];
        double mass=1-ts[end-1];
        auto lo=begin,hi=end-1;
        while(lo<end-1&&(1-ts[lo])<.05*mass)++lo;
        while(hi>lo&&(1-ts[hi-1])>=.95*mass)--hi;
        const float* at=std::lower_bound(zs+lo,zs+hi+1,depth[i]);
        if(at<zs+hi+1&&std::abs(*at-depth[i])<=2*delta)support[i]=1;
        if(at>zs+lo&&std::abs(*(at-1)-depth[i])<=2*delta)support[i]=1;
    }
}
