...
The \textbf{corrector} is precise and 2D: per-view \textbf{Canny} edge maps of the training
photographs (unblurred, hysteresis 50/150), converted to sub-pixel \textbf{distance-transform
(DT) fields}, one per training view. A learned detector enters only through the \emph{seed
...
matched-precision configurations of \S4.1--4.4, \textbf{TEED} --- a frozen, zero-shot,
...
opacity > 0.1 and k-NN (k=8) mean spacing < 3$\times$ its median. Pull field: Canny, no blur,
hysteresis 50/150, distance-transformed (every configuration). Seed ranking: Canny-sourced
(blurred two-scale union, $\sigma$ = 2.0 and 2.5, thresholds 100/200 and 75/150) for \S5.1 and \S4.5;
TEED-sourced (native scale, NMS-thinned, threshold 0.5) for the sweeps of \S4.1--4.4. DT
...
seeds in the \S4.1--4.4 configurations (the pull field is Canny throughout). The multi-view
DexiNed triangulation and the DINOv2 probe of \S5 and Appendix A are \emph{diagnostic
...
The crease set is also a topology-selected subset of the geometric $\geq$30$^{\circ}$ edges --- \textbf{37 \%
...
\textbf{A learned edge field would not lift the ceiling either.} Its closed-form upper bound
fails a pre-registered gate on both scenes (lego AUC \textbf{0.698}, recall \textbf{0.000} at 85 \%
precision; chair \textbf{0.859 / 0.000}) --- the detector's $\approx$5 px response tail, not missing
signal (Appendix A.4).
