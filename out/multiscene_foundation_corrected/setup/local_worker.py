...
for scene in scenes:
 ...
 while not all(p.exists() for p in required):
  ...
  time.sleep(10)
 eligibility=prepare_eligibility(out,scene,cfg);print('eligibility',scene,eligibility,flush=True)
 ...
 for task,asset in jobs:
  ...
  env=os.environ.copy();env.update(CUDA_VISIBLE_DEVICES=str(queue),OMP_NUM_THREADS='4',OPENBLAS_NUM_THREADS='4',MKL_NUM_THREADS='4',PYTHONDONTWRITEBYTECODE='1')
