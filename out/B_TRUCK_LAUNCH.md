...
## Update 2026-09-15 16:0x (fire) — 2DGS RETRACTED for method-identity; 3DGS Truck trained + field-verified
- **Method-path correction (not tuning):** the 2DGS Truck ply lacks scale_2 (2DGS is 2-scale by construction), so it is NOT byte-identical to the crown load_gaussians path (which reads scale_0/1/2, rot_0..3, f_dc_0..2, opacity). Feeding it would silently change the carrier definition. Per the audit rule (recipe byte-identical, only data source changes) 2DGS is RETRACTED as the exp-B carrier. The crown claim is a 3DGS claim; the real-pose test must use 3DGS.
- **Frozen vanilla 3DGS on Truck TRAINED** (shipped 3DGS recipe, authors COLMAP defaults, 30k iters, eval on): final train loss ~0.037; point_cloud/iteration_30000 on disk (gitignored). 2,582,093 Gaussians.
- **Field-identity VERIFIED (driver-side, cheap):** ply properties checked against src/common.load_gaussians requirements -> MISSING: none. The ply half of M1b ingestion is now byte-identical to crown carriers. Only the CAMERA source differs (COLMAP per-image intrinsics 1957x1091 vs synthetic shared camera_angle_x) + real rgb_paths.
- **Remaining blocker UNCHANGED (honest):** the shipped load_cameras() reads Blender transforms_train.json. A camera-only adapter (COLMAP cams -> src.common.Camera, wire trained ply + real rgb_paths into extract_seeds) is still required. It touches the method-path boundary and MUST be design-argued with the agent before writing (guarantee the OVERALL photometric score recipe is unchanged; only data source swaps). NOT written this fire.
- dss9 agent STILL login-expired (OAuth, since ~9:42 Tokyo; all /login interrupted; cannot restore headless). Pre-registered go/no-go UNCHANGED and frozen. No fabrication.
