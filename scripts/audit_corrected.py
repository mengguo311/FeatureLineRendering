...
result=dict(passed=all(r['passed'] for r in rows),trace_count=len(rows),forbidden_successes=sum(len(r['forbidden_successes']) for r in rows),unparsed_open_lines=sum(len(r['unparsed_open_lines']) for r in rows),stages=rows,scope='Kernel native-open audit includes startup, with explicit metadata/source-cache exceptions. Landlock precedes scientific decoding and inference; approved input bytes may be hashed during allowlist construction. Evaluation is read-only on method roots; DEV photography only after output freezing. No TEST, mesh, learned-normal/depth cache, or historical scientific array is allowed.')
