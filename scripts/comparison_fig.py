...
CAPTION = (
    ...
    "method path.\n"
    "The poster derives PER-FRAME IMAGE-SPACE lines from the rasterization state itself (opacity, depth, normal, "
    "albedo, top-k Gaussian IDs); ours builds a\n"
    "STATIC OBJECT-SPACE stroke set from multi-view photographic edge evidence anchored on the gaussian cloud and "
    "renders it through the 3DGS z-buffer.\n"
    "The poster is the closest mesh-free per-frame comparator, NOT a method we extend: none of its fields is "
    "computed or consumed in our pipeline. We have not\n"
    "seen the poster, and its future-work statement is not on record.\n\n"
    "DISCLOSURE -- the two rows use DIFFERENT ASSETS and this is NOT a same-asset head-to-head. The poster row "
    "is the lego bulldozer (our choice of asset; the poster's\n"
    "asset is not on record); our row is cadpartA\n"
